...
## Phase 0G Input-Source Policy

**Resolves `P0G-B002` (manual context fallback vs. automatic detection) and documents the input-source consequences of `P0G-B003` (HH/HL/LH/LL/BOS/CHoCH deferral), `P0G-B004` (Equal High/Low lifecycle deferral), and `P0G-B005` (Trendline lifecycle deferral).** See `knowledge/FINAL_PHASE_0G_KNOWLEDGE_GAP_AUDIT.md`, "Post-Audit Author Decisions — P0G-B002 through P0G-B007 and P0G-B013," for the full decision record. **No state, gate, transition, or cancellation condition defined above is changed by this section.**

- **Reviewed manual expert context labels are permitted.** The Context Gate's existing `market_direction_status` and `analytical_framework_status` fields (SS "Context Gate" above) may continue to be populated from expert labels, approved external context modules, or explicit manual review, per the existing allowance already documented there (`context_input_source = MANUAL_EXPERT_LABEL`). Where a reviewed label additionally records qualitative context evidence, the permitted supplementary vocabulary is `context_direction = BULLISH | BEARISH | NEUTRAL | UNCLEAR` and `context_alignment = ALIGNED | MISALIGNED | UNCLEAR` — these are reviewed-evidence annotations only and do not replace or redefine `market_direction_status`/`analytical_framework_status` as the actual gate-determining fields.
- **Automatic trend and market-structure detection is deferred.** Per `P0G-B002`/`P0G-B003`, no automatic HH/HL/LH/LL/BOS/CHoCH rule, moving-average rule, or other automatic market-direction/analytical-framework detector is required for Phase 0G knowledge-gate closure, is approved, or is defined by this section. An automatic detector remains a future requirement for autonomous (non-manually-reviewed) operation, not yet approved.
- **Reviewed manual Equal High/Low liquidity-event labels may be used** as `LIQUIDITY_BEFORE_POI`/`LIQUIDITY_WITHIN_POI`/`LIQUIDITY_AFTER_POI` evidence under the existing Liquidity Gate above, tagged `liquidity_event_source = MANUAL_EXPERT_LABEL` (see `knowledge/poi_rules/structural/equal_highs.md` and `equal_lows.md`, "Phase 0G Specialized Lifecycle Deferral"). The Equal High/Low structure alone, without a reviewed label, does not satisfy the Liquidity Gate.
- **Reviewed manual Trendline-event labels may be used** as analytical context, tagged `trendline_event_source = MANUAL_EXPERT_LABEL` (see `knowledge/poi_rules/structural/bullish_trendline.md` and `bearish_trendline.md`, "Phase 0G Specialized Lifecycle Deferral"). An unresolved Trendline event, without a reviewed label, does not satisfy any BTMM gate.
- **Source fields must remain visible.** `context_input_source`, `liquidity_event_source`, and `trendline_event_source` (and the pre-existing `liquidity_evidence_source`) are preserved and exposed, never silently dropped or merged into a composite field.
- **A structure alone cannot automatically satisfy a reviewed-event gate.** Equal Highs, Equal Lows, and Trendlines require an explicit reviewed manual label to serve as gate evidence; their mere detection/confirmation (per their own existing formation/confirmation rules) is not itself sufficient.
- **Manual evidence must not be represented as automatic detection.** Every field populated via `MANUAL_EXPERT_LABEL` must record that source explicitly; it must never be reported, displayed, or logged as though it were produced by an automatic detector.
- **Existing mandatory BTMM gates remain unchanged** — POI, Context (Market Direction/Analytical Framework/Active Session), Liquidity, Accuracy, Volume Pillar, Reaction, Reaction Speed, and Formation Timeframe all remain exactly as defined above; no gate was added, removed, or reweighted.
- **No HH, HL, LH, LL, BOS, or CHoCH requirement is introduced** by this section, by `P0G-B002`, or by `P0G-B003`.
