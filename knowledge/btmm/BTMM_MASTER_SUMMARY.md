...
## Phase 0G Input-Source Policy (P0G-B002 through P0G-B007 and P0G-B013, Resolved)

Full specification: [BTMM_STATE_MACHINE.md](BTMM_STATE_MACHINE.md), "Phase 0G Input-Source Policy." Full decision record: `knowledge/FINAL_PHASE_0G_KNOWLEDGE_GAP_AUDIT.md`, "Post-Audit Author Decisions — P0G-B002 through P0G-B007 and P0G-B013." **No BTMM state, gate, transition, or cancellation condition is changed by these decisions.**

- **Reviewed manual expert context labels are permitted** for the Context Gate's `market_direction_status`/`analytical_framework_status` fields (`context_input_source = MANUAL_EXPERT_LABEL`); automatic trend and market-structure detection (including any HH/HL/LH/LL/BOS/CHoCH rule) is formally deferred and not required for Phase 0G closure.
- **Reviewed manual Equal High/Low liquidity-event labels** (`liquidity_event_source = MANUAL_EXPERT_LABEL`) and **reviewed manual Trendline-event labels** (`trendline_event_source = MANUAL_EXPERT_LABEL`) may be used as evidence; neither structure alone (without a reviewed label) satisfies a BTMM gate. Equal Highs/Equal Lows and both Trendlines each retain their own formally deferred specialized lifecycle — see their individual rule files' "Phase 0G Specialized Lifecycle Deferral" sections.
- **POI freshness and age** are now tracked descriptively only, per `knowledge/poi_lifecycle/POI_FRESHNESS_AND_AGE_STANDARD.md` (`FRESH`/`INTERACTED`/`NOT_AUTOMATICALLY_EVALUATED`/`NOT_APPLICABLE`; descriptive age fields with `automatic_age_expiration = DISABLED`). Freshness and age never automatically affect strength, validity, lifecycle status, BTMM eligibility, entry validity, invalidation, or expiration.
- **Negative-example sufficiency (`P0G-B013`):** positive examples, explicit mandatory formation rules, and explicit rejection criteria are accepted as sufficient evidence for Phase 0G; a comprehensive negative/counter-example dataset remains deferred to the annotation and validation phase (`P0G-B021`).
- Source fields (`context_input_source`, `liquidity_event_source`, `trendline_event_source`, `liquidity_evidence_source`) must remain visible and must never be represented as automatic detection.
