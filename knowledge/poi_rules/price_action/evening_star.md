...
A three-candle reversal pattern at supply/resistance: strong bullish candle, then a small/doji candle, then a strong bearish candle - a full volume switch from buyers to sellers.

## Required market location

Around a supply zone or resistance structure.

## Formation conditions

Candle 1: strong bullish. Candle 2: small candle or doji. Candle 3: strong bearish.
...
The middle doji candle's area is explicitly called out as the POI itself. **Resolved (GROUP3-D8, Author-Approved):** the POI is the complete High-to-Low range of the qualifying middle Doji candle only — candle 1 and candle 3 are not included in the zone; candle 3 remains pattern-confirmation evidence only.
...
Candle 2 must be "small" or a doji. **Resolved (GROUP3-D7, Author-Approved — Morning/Evening Star Doji Threshold):** `middle_candle_body_efficiency = ABS(Middle Close − Middle Open) / (Middle High − Middle Low)`. The middle candle is invalid when `Middle High − Middle Low <= 0`. **Standard Doji:** `middle_candle_body_efficiency <= 0.10`. **Strong Doji shape tier:** `middle_candle_body_efficiency <= 0.05`. Evening Star requires a qualifying Doji as candle two; the middle candle may close bullish or bearish, and middle-candle colour is not decisive when the threshold passes.

## Candle-size requirement

Size Ratio (Total Range) of candle 3 vs. candle 2 ideally >= 2.0-3.0, per Measurement Standard V1.
...
Not defined beyond appearing at supply/resistance.
...
Not defined as Evening-Star-specific.
...
Only the >=2-3x ratio on the final candle is given; no separate standard/strong labels.
...
**Non-repainting timing:** all inherited lifecycle events (breach, reclaim, displacement, false/genuine invalidation) become available only after their complete conditions are confirmed, per the shared standard, and never before `evening_star_poi_available_time`.
...
Uniquely introduces the 'doji as a POI' idea. This overlap observation remains open and low-priority; no precedence rule has been created.
