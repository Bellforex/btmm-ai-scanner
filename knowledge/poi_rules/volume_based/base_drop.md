...
**Zone Top mapping:** Base High = highest high of all base candles (the already-approved Base Drop boundary — unchanged by this inheritance, used exactly as documented above).
...
**Lifecycle availability time (RECON-D2, approved):** `base_available_time = qualifying_departure_candle_close_time` — the close time of the first confirmed departure candle satisfying the approved Base Formation Standard (bearish close, close below Base Low, Size Ratio >= 2.0/3.0 versus the largest base candle, decisive separation from the base). The POI becomes lifecycle-eligible **only** at this time — **not** at the first base candle, **not** at the final base candle, and **not** at a future first return to the zone. This decision does not change base candle count, compactness, Base Height, Overlap Ratio, departure-size ratio, directional close requirements, boundaries, or strength classification.
