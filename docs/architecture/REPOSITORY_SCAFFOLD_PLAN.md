...
## 26. POI Detection and Lifecycle Foundation — Architecture (Author-Approved)

**Status: `AUTHOR-APPROVED`, `APPROVED FOR CONTROLLED IMPLEMENTATION`, `NOT YET IMPLEMENTED`, `NOT PRODUCTION-APPROVED`.** Full detail: `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §35, **as corrected by one focused read-only architectural audit and one consolidated documentation-only correction pass, and now approved by the author in full (register §35AP).** **This is the exact milestone reserved by every prior closure (§25 above; register §33AA item 2/§34AN; `1B-H-MEASUREMENTS` and `1B-I-STRUCTURE`'s own "next controlled action" text).** Title includes "Lifecycle," not "Detection" alone, because the milestone implements the already-approved POI Boundary Breach/Reclaim/Invalidation standard (Ambiguity 15) and the already-approved Freshness/Age standard as public lifecycle-transition records for **exactly 18** of the 36 POI specifications (corrected from an original miscount of "30"/"32").
...
- **Exact scope (proposed, not yet approved):** 35 changed paths — 18 new source files in a new top-level package (already reserved above), 16 new test files, 1 modified existing file (`domain/enums.py`, +3 enum members). New-path split 18 source/16 test; affected-path split 19 source/16 test (both stated explicitly, never conflated). 120 new top-level test functions, redistributed (500 combined with the existing 380). Proposed creation order **113-146**. **23 new public exports** (corrected from an internally-inconsistent claim of 24; no `domain`/`structure` re-exports). No dependency change; no `market_data`/`domain` Protocol modification; `structure` is no longer a dependency at all. Register §35AH/§35AI/§35AJ.
- **Author-approved, exactly as corrected — no modification was made to any corrected element (register §35AP).** No code, test, dependency, or package file exists yet for this milestone; this remains a documentation-only approval. **Next step:** implement all 35 approved paths in one complete controlled cycle (no per-file decision groups), followed by one final architectural audit, at most one correction cycle for a genuine defect, one implementation commit, and one compact closure commit. **No networking, real file parsing, production persistence, BTMM detection, entry logic, visualization, or robot work is authorized by this approval.**
