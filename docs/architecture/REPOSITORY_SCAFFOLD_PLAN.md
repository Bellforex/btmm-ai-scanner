...
- **Phase 1B creation:** first content implemented and closed by the Market Measurements and Reference Structures Foundation milestone (§24, register §33AC) — `ConfirmedSwing`, `DisplacementObservation`, `EqualLevelCluster`, `SupportResistanceZone`, `Trendline`. HH/HL/LH/LL/BOS/CHoCH content remains prohibited here (`P0G-B003`) — a new, separate top-level package (`structure/`, below), implemented and closed by the Structure State and Transition Foundation milestone (§25, register §34Z), was created for it instead, since it would otherwise contradict this entry's own "Must not contain" line. `domain/enums.py`'s `DerivedOutputType` alone gains a narrow, author-approved exception — 3 routing members reused by `structure/` — no BOS/CHoCH *value* enum exists in `domain/` itself.
...
- **Phase 1B creation:** not previously reserved in this document; introduced by the Structure State and Transition Foundation milestone (§25, register §34) — `AUTHOR-APPROVED`, `IMPLEMENTED`, `VERIFIED`, `ARCHITECTURALLY AUDITED`, `COMMITTED`, `PUSHED`, `CLOSED`, `NOT PRODUCTION-APPROVED`. Implementation commit `74fd50c6a44c208153769d2f1947bdfa7ff0d3cf`.

### `src/btmm_ai_scanner/poi/`
- **Purpose:** The 36 POI type representations and their formation/boundary rules.
- **May contain:** POI record construction per each POI's approved specification. Per the implemented `1B-J-POI` architecture (§26, register §35, **corrected by the focused architectural audit and one consolidated correction pass; author-approved, implemented, verified, architecturally audited, committed, pushed, closed — register §35AP/§35AQ**), this concretely means: `PoiObservation`, `PoiLifecycleTransition`, `PoiOverlapRelationship`, `CurrentPoiState`, `PoiAnalysis`, `PoiConfiguration`, `PoiFamily`/`PoiDirection`/`PoiType`/`PoiStrengthTier`/`PoiLifecycleStatus`/`PoiFreshnessStatus` (corrected: no `PoiValidityStatus` — removed entirely, register §35D/§35U), and `analyze_pois()`, for exactly 32 of the 36 approved POI specifications (18 of which are lifecycle-eligible; 14 permanently `NOT_APPLICABLE`).
- **Must not contain:** trade placement of any kind. **POI detectors must never place trades.** Per the implemented `1B-J-POI` architecture: no BTMM manipulation lifecycle, entry signal, stop loss, take profit, position sizing, or trade-outcome field of any kind. No Trendline-as-POI or Swing-High/Swing-Low-as-POI content (both deferred, register §35C) — none was added to `PoiType`, source detectors, exports, or tests as implemented behavior.
- **Allowed dependency direction — corrected:** `domain`, `measurements`, `contracts`, `config`. **Corrected by the focused audit (register §35AH):** `structure` is removed entirely (no implementable POI type's approved rule required `StructureAnalysis`/`CurrentStructureState` for any gating purpose — a required input with no deterministic effect, register §35B/§35S); `contracts` remains added, since it is genuinely needed for `ContractModel`/`SemVer`/`SHA256Fingerprint`/`UUIDv7`. Author-approved (register §35AP) and implemented exactly as approved (register §35AQ).
- **Phase 1B creation:** introduced by the POI Detection and Lifecycle Foundation milestone (§26, register §35) — **`AUTHOR-APPROVED`, `IMPLEMENTED`, `VERIFIED`, `ARCHITECTURALLY AUDITED`, `COMMITTED`, `PUSHED`, `CLOSED`, `NOT PRODUCTION-APPROVED`.** Implementation commit `ebc55e0bdbbef7b7f528d874107daa2d75a5628a`.
...
## 25. Structure State and Transition Foundation — Architecture (Implemented, Closed)

**Status: `AUTHOR-APPROVED`, `IMPLEMENTED`, `VERIFIED`, `ARCHITECTURALLY AUDITED`, `COMMITTED`, `PUSHED`, `CLOSED`, `NOT PRODUCTION-APPROVED`.** Full detail: `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §34Y/§34Z. **This is the exact milestone reserved by `1B-H-MEASUREMENTS` (§24 above, register §33B/§33AB item 2; `P0G-B003`). No prior document defines any HH/HL/LH/LL/BOS/CHoCH/protected/weak rule — every rule is newly authored in register §34. This section was a consolidated, documentation-only correction of the originally proposed architecture, resolving every blocking finding from the focused read-only architectural audit in one pass; the author approved the corrected architecture in full (register §34Y), with no modification to any corrected element, and it has since been implemented, verified, architecturally audited, committed, and pushed exactly as approved (register §34Z).**
...
- **Author-approved, exactly as corrected — no modification was made to any approved element (register §34Y).**
- **Implementation closure (register §34Z):** all 16 approved paths (7 new source files, 8 new test files, 1 modified existing file) were implemented in one complete cycle, passing every quality gate (`uv lock --check`, `ruff format --check .`, `ruff check .`, `mypy src tests`, full suite 458 passed, original baseline 34 passed, 60 new top-level test functions, 380 combined) with no dependency or `market_data`/`domain` Protocol change.
- **Final architectural audit:** the one authorized correction cycle fixed Ruff and mypy findings only — an unused import in `structure/analyzer.py`, an unused import and a concatenation-style nit in two test files, loop-variable name reuse across differently-typed loops in `structure/transitions.py`, and one `type: ignore` code correction. No architecture decision, contract, test name, test count, path, export, dependency, or Protocol changed. No other defect was found. Implemented and committed as `74fd50c6a44c208153769d2f1947bdfa7ff0d3cf`, pushed to `origin/main`. Full detail: register §34Z.
- **No network adapter, real file parser, production persistence backend, POI detector, BTMM detector, indicator, alert, backtester, or robot was implemented.** The scaffold has advanced from author approval (§34Y) to a completed, closed Structure State and Transition Foundation, and no further.
- **Phase `1B-I-STRUCTURE` is closed but not production-approved.** No production, live-trading, indicator, robot, provider-networking, or persistence-backend approval exists or is implied by this closure. **Next controlled action:** define the **POI Detection Foundation**, using the completed measurement and structure foundations — POI candidate identity, POI type taxonomy, order-block candidates, fair-value-gap candidates, support/resistance and trendline POI references, equal-level liquidity references, candlestick-pattern POIs, structural context requirements, strong-timeframe precedence, overlap/merge rules, confirmation, validity, breach, reclaim, invalidation, availability time, no-look-ahead behavior, and deterministic identity/fingerprinting/replay equivalence — without yet including BTMM manipulation lifecycle, entry signals, stop loss, take profit, position sizing, visualization, Telegram alerts, broker execution, AI inference, or production approval. Not started by this closure.

## 26. POI Detection and Lifecycle Foundation — Architecture (Implemented, Closed)

**Status: `AUTHOR-APPROVED`, `IMPLEMENTED`, `VERIFIED`, `ARCHITECTURALLY AUDITED`, `COMMITTED`, `PUSHED`, `CLOSED`, `NOT PRODUCTION-APPROVED`.** Full detail: `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §35AP/§35AQ. **This is the exact milestone reserved by every prior closure (§25 above; register §33AA item 2/§34AN; `1B-H-MEASUREMENTS` and `1B-I-STRUCTURE`'s own "next controlled action" text).** Title includes "Lifecycle," not "Detection" alone, because the milestone implements the already-approved POI Boundary Breach/Reclaim/Invalidation standard (Ambiguity 15) and the already-approved Freshness/Age standard as public lifecycle-transition records for **exactly 18** of the 36 POI specifications (corrected from an original miscount of "30"/"32"). This section was a consolidated, documentation-only correction of the originally proposed architecture, resolving every blocking finding from the focused read-only architectural audit in one pass; the author approved the corrected architecture in full (register §35AP), with no modification to any corrected element, and it has since been implemented, verified, architecturally audited, committed, and pushed exactly as approved (register §35AQ).

This milestone gives the `poi/` directory (Section 3, above — "Directory only" through `1B-I-STRUCTURE`) its first proposed content: a deterministic, no-look-ahead transformation of `NormalizedCandle` sequences plus `MarketMeasurementAnalysis` (per timeframe) into confirmed POI observations, lifecycle transitions, overlap/merge relationships, and a current-state snapshot per POI instance. **Corrected: `StructureAnalysis` is not part of this transformation at all** — the focused audit found no implementable POI type's approved rule required it, making it a required input with no deterministic effect; it is removed entirely, not merely made optional.

- **36-POI coverage and implementability gate:** all 36 approved book POI specifications audited against the full knowledge base (`knowledge/POI_MASTER_CATALOG.md`, `knowledge/POI_COVERAGE_MATRIX.md`, all 36 `knowledge/poi_rules/` files, all 5 `knowledge/poi_lifecycle/` standards, `knowledge/btmm/BTMM_STATE_MACHINE.md`, `knowledge/market_structure/MARKET_ANALYSIS_COVERAGE_MATRIX.md`). **32 IMPLEMENTABLE_WITH_AUTHOR_GAP_FILL; 4 DEFERRED (Bullish/Bearish Trendline — no approved finite zone geometry; Swing High/Swing Low — already fully represented by the existing, closed `domain.ConfirmedSwing` contract, not duplicated); 0 BLOCKED.** Full 36-row matrix: register §35C.
- **Corrected lifecycle-eligibility split: exactly 18 of the 32 implementable types are `FULL` (lifecycle-eligible — 10 volume + 6 price-action + Support + Resistance); exactly 14 are permanently `NOT_APPLICABLE`** (Equal Highs, Equal Lows, all 12 period-level types). Register §35C/§35D/§35V.
- **No volume-based POI is blocked by missing volume data — explicitly Option B (price-action/displacement proxy)** — all 10 volume-based specifications are fully detectable via already-approved price-action proxies; `NormalizedCandle.volume`/`volume_kind` (already implemented) is strictly secondary, non-mandatory evidence; the four proxy fields are computed internally only, never stored on `PoiObservation` or exported. Register §35M/§35Q.
- **Five separated concepts, never conflated (`docs/PROJECT_SCOPE.md` §7, unmodified):** POI detection/confirmation (no separate "validity" axis, corrected); POI lifecycle status (18 eligible types only); BTMM-pattern validity (out of scope); entry validity (out of scope); trade outcome (out of scope). Register §35B.
- **Taxonomy:** `PoiFamily` (3 members), `PoiDirection` (2 members, no `NEUTRAL`), `PoiType` (exactly 32 members, no placeholder for any deferred specification) — **corrected: three separated status axes, not four** (`PoiValidityStatus` removed entirely). Register §35D.
- **Core contract:** one immutable `PoiObservation`, **corrected to exactly 23 fields** (removed: `validity_status`, `source_structure_record_ids`), with `representative_price`/zero-height-point-zone treatment for the 12 single-price-point period levels, and a disclosed content-evolving-snapshot exception for exactly 6 "Current" period-level types. Register §35E.
- **Identity:** `DerivedOutputIdentityProvider` reused structurally unmodified, 3 new `DerivedOutputType` members (`POI_OBSERVATION`, `POI_LIFECYCLE_TRANSITION`, `CURRENT_POI_STATE`) appended to `domain/enums.py` — the one modified existing path. Register §35F.
- **Fingerprint strategy: Option C** — a third disclosed, tested-equivalent duplicate of the canonical serializer, now with a required three-way cross-package equivalence test. Register §35G.
- **Input model — corrected:** immutable per-timeframe input bundles (`PoiTimeframeInput`, `structure_analysis` field removed entirely); an explicit, exact validation/error list narrowed accordingly. Register §35H.
- **Timeframe policy:** the existing 8-member `Timeframe` enum is sufficient (no H6/H8/H12 exist anywhere in this project); an explicit total timeframe-strength order, explicitly stated as **not** a resolution of `P0G-B014` and never labeling H1/M15 as strong/weak; the unused `strong_poi_timeframes` configuration field is removed. Register §35I.
- **Per-family architecture** for Fair Value Gaps, Order Blocks, the six candlestick patterns, Buy-to-Sell/Sell-to-Buy/Base/Pressure-Wick, Support/Resistance references, and the 12 period levels — each reusing an already-approved formula, with every remaining gap explicitly disclosed rather than silently invented (e.g., no automated Order-Block "origin vs. middle" gate; the calendar-window boundary for the 12 period levels is now fully specified as an exact UTC policy requiring only author ratification). Register §35J-§35R.
- **Trendline and Equal-High/Low treatment:** Trendline is deferred entirely (no `PoiType` member); Equal Highs/Lows are explicitly **liquidity-reference POIs, not entry signals**, detected but carrying a permanently fixed `PoiLifecycleStatus = NOT_APPLICABLE` (no sweep lifecycle invented, per the already-approved `P0G-B004` Option B deferral). Register §35O/§35P.
- **Exact UTC period-window policy (resolving the prior open gap-fill, now author-approved):** day `[00:00:00 UTC, next day 00:00:00 UTC)`; ISO week `[Monday 00:00:00 UTC, next Monday 00:00:00 UTC)`; calendar month `[1st 00:00:00 UTC, 1st-of-next-month 00:00:00 UTC)`; no broker-local timezone; no DST; weekend/holiday windows simply empty; previous-period resolution skips empty windows; current-period content evolves under a stable identity. `ENGINEERING-PROVISIONAL`, not broker-session-calibrated, not production-approved. Register §35R/§35AP.
- **No structural input of any kind** for any implementable POI type — `StructureAnalysis` removed entirely from this milestone (corrected from "optional, non-gating fields"); a future BOS/CHoCH cannot influence any POI, since no structure fact is ever supplied. Register §35S.
- **Confirmation/lifecycle, cleanly separated — corrected, no third "validity" axis:** an internal candidate is private and unexported; a `PoiObservation`'s existence is itself the confirmation signal; the full breach/reclaim/displacement/false-invalidation/sustained-breach/genuine-invalidation/failed-reclaim/repeated-tap state machine (with the breach-candle-excluded-from-reclaim-count nuance now stated explicitly) reused verbatim from the already-approved Ambiguity 15 standard, applicable to exactly 18 types; the already-approved observational-only freshness/age model (`FRESH`/`INTERACTED`, no automatic degradation or expiration). `EXPIRED` is absent from every public enum pending a future, separate expiration decision. Register §35T-§35W.
- **Overlap and merge — corrected with exact zero-height point-zone rules:** strict interval-intersection overlap test for two non-zero-height zones, distinct from boundary-touching and containment; a point overlaps an interval when `bottom <= P <= top` (boundary-inclusive); two points overlap only when exactly equal; same-`poi_type` cross-timeframe overlap merges into the stronger timeframe; cross-`poi_type` or same-timeframe overlap is reported (`PoiOverlapRelationship`) but never merged; bullish/bearish overlapping zones create no relationship at all; no transitive overlap inference. Register §35X.
- **Output model:** `PoiObservation`, `PoiLifecycleTransition`, `PoiOverlapRelationship`, `CurrentPoiState` (corrected to 21 fields), and the `PoiAnalysis` aggregate — no `MergedPoi` fifth contract, no BTMM/entry/trade-outcome field anywhere. Register §35Z.
- **Public API:** `analyze_pois(timeframe_inputs, configuration, identity_provider) -> PoiAnalysis`, one entry point, no per-family analyzer, `structure_analysis` removed. Register §35AA.
- **Exact scope:** 35 changed paths — 18 new source files in a new top-level package (already reserved above), 16 new test files, 1 modified existing file (`domain/enums.py`, +3 enum members). New-path split 18 source/16 test; affected-path split 19 source/16 test (both stated explicitly, never conflated). 120 new top-level test functions (500 combined with the existing 380). **Creation order 113-146.** **23 new public exports** (corrected from an internally-inconsistent claim of 24; no `domain`/`structure` re-exports). No dependency change; no `market_data`/`domain` Protocol modification; `structure` is not a dependency at all. Register §35AH/§35AI/§35AJ.
- **Author-approved, exactly as corrected — no modification was made to any approved element (register §35AP).**
- **Implementation closure (register §35AQ):** all 35 approved paths (18 new source files, 16 new test files, 1 modified existing file) were implemented in one complete cycle, passing every quality gate (`uv lock --check`, `ruff format --check .`, `ruff check .`, `mypy src tests`, full suite 578 passed, original baseline subset 34 passed, 120 new top-level test functions, 500 combined) with no dependency or `market_data`/`structure` Protocol change.
- **Final architectural audit:** verdict `B — PASS WITH NON-BLOCKING FINDINGS — READY TO COMMIT`. One genuine defect was found and corrected before commit — a lifecycle-status-reset defect in `poi/lifecycle.py` that unconditionally discarded the true terminal-for-that-event status after a breach event resolved; corrected inside the approved 35-path scope, with no architecture, contract, test-count, export-count, dependency, or Protocol change. One disclosed non-blocking finding — the approved no-look-ahead behavior is implemented as availability-time-driven correctness by construction rather than a literal procedural six-phase loop, matching the precedent already accepted for `domain/analyzer.py` and `structure/analyzer.py`; verified correct via replay, determinism, and activation-immunity tests, and it did not require a second correction cycle. No other defect was found. Implemented and committed as `ebc55e0bdbbef7b7f528d874107daa2d75a5628a`, pushed to `origin/main`. Full detail: register §35AQ.
- **No network adapter, real file parser, production persistence backend, BTMM detector, indicator, alert, backtester, or robot was implemented.** The scaffold has advanced from author approval (§35AP) to a completed, closed POI Detection and Lifecycle Foundation, and no further.
- **Phase `1B-J-POI` is closed but not production-approved.** No production, live-trading, indicator, robot, provider-networking, or persistence-backend approval exists or is implied by this closure. **Next controlled action:** define the **BTMM Manipulation Lifecycle Foundation** (proposed identifier `1B-K-BTMM`), using the completed market-data, measurement, structure-state, and POI detection and lifecycle foundations — POI interaction as a required boundary, manipulation lifecycle states, accumulation/distribution context, liquidity before/within/after POI interaction, approach/first-contact/overshoot/breach/reclaim/rejection, reaction strength, market speed and displacement, pressure behavior, equal-level liquidity interaction, protected/weak structure references, BOS/CHoCH interaction, buy-to-sell/sell-to-buy manipulation, lifecycle inheritance and conditional reconciliation, POI breach/reclaim/invalidation inheritance, deterministic no-look-ahead transition ordering, historical replay equivalence, and the explicit separation of POI validity, BTMM-pattern validity, entry validity, and trade outcome — without yet including live entry execution, stop loss, take profit, position sizing, trade management, visualization, Telegram alerts, backtesting statistics, broker connectivity, MT5/MT4, AI inference, or production approval. Not started by this closure.

## 27. BTMM Manipulation Lifecycle Foundation — Architecture (Implemented, Closed)

**Status: `AUTHOR-APPROVED`, `IMPLEMENTED`, `VERIFIED`, `ARCHITECTURALLY AUDITED`, `COMMITTED`, `PUSHED`, `CLOSED`, `NOT PRODUCTION-APPROVED`.** Full architecture: register §36 (corrected), closure §36AU. Full file scope: `PHASE_1B_EXACT_SCAFFOLD_FILE_SCOPE.md`, Section 41. This section was a consolidated, documentation-only correction of the originally proposed architecture, resolving every finding from the focused architectural audit and a subsequent narrow consistency review; the author approved the corrected architecture in full (register §36AT), with no modification to any corrected element, and it has since been implemented, verified, architecturally audited, committed, and pushed exactly as approved (register §36AU).

- **Foundational finding — no separate BTMM pattern taxonomy (unchanged):** no approved "Buy-to-Sell manipulation"/"Sell-to-Buy manipulation" pattern concept exists distinct from generic `BtmmDirection` (`BULLISH_BTMM`/`BEARISH_BTMM`, 2 members). The existing `BUY_TO_SELL_CANDLE`/`SELL_TO_BUY_CANDLE` `PoiType` values already fully represent these concepts; no `BtmmPatternType` enum is created. Register §36C.
- **Corrected finding — `BTMM_CONFIRMED` is now genuinely reachable:** the original draft found `BTMM_CONFIRMED` structurally unreachable because the Context Gate (Market Direction, Analytical Framework, Active Session), the full Liquidity Gate, and the Volume Pillar Gate had no input channel. **Corrected:** one new, explicit, caller-supplied `BtmmReviewedEvidence` input contract (register §36G2) now carries already-reviewed evidence for exactly these gates, matching the approved `BTMM_STATE_MACHINE.md` "Phase 0G Input-Source Policy" (`context_input_source = MANUAL_EXPERT_LABEL` and equivalents) — no automatic context detector, session calendar, or Volume Pillar formula is invented; the milestone only accepts already-reviewed facts as typed input. `BTMM_CONFIRMED` is reached at `FINAL_GATE_EVALUATION` when all 10 gates resolve favorably simultaneously (5 deterministic + 5 via reviewed evidence). Register §36D/§36S/§36I5.
- **Volume Pillar Gate: Option B, final** — no automatic formula is implemented (Option A, proxy-field reuse, is permanently removed from consideration); the gate resolves only through `BtmmReviewedEvidence.volume_pillar_status`. A new, dedicated 5-member `BtmmVolumePillarStatus` enum (`PENDING`/`SUPPORTS`/`FAILS`/`MISSING_DATA`/`UNRESOLVED`) replaces the incorrect reuse of the generic `BtmmGateStatus` for this field, matching `BTMM_STATE_MACHINE.md`'s own exact vocabulary. Register §36H/§36R.
- **Reaction Gate timing corrected:** the gate no longer resolves `PASS`/`FAIL` before the complete five-candle reaction window closes, even when thresholds are numerically satisfied early — the highest reaction tier achieved anywhere across the full window is selected only at the fifth confirmed candle, matching `MEASUREMENT_STANDARDS.md`'s own "assign the highest reaction tier achieved during the window" rule exactly and eliminating a premature-confirmation/repainting risk found during the audit. Register §36N.
- **Lifecycle model fully specified, corrected:** initial `primary_state = BTMM_CANDIDATE` is now explicit; an `ENTERED_FORMING` transition (disclosed ENGINEERING-PROVISIONAL author gap-fill, since the approved standard names no distinct transition identifier for this step) models `BTMM_CANDIDATE → BTMM_FORMING`; a single generic `BLOCKED` transition (carrying its reason inline, matching the approved standard's own "reason preserved separately" design) and a `RESUMED_FORMING` transition model the `BTMM_FORMING ⇄ BTMM_BLOCKED` loop, with an explicit idempotency guarantee (no repeated transition emitted for an unchanged blocking condition); a `CONFIRMED` transition models reaching `BTMM_CONFIRMED`, reachable only from `BTMM_FORMING`, never directly from `BTMM_BLOCKED`. `BtmmLifecycleTransitionType` grows from 9 to 15 members; `BtmmCancellationReason` grows from 4 to 8 members (adding `CONTEXT_REJECTED`, `SESSION_INACTIVE`, `VOLUME_PILLAR_FAILED`, `NO_LIQUIDITY_EVIDENCE`, all now reachable via reviewed evidence). One exact closed transition table covers every state pair, trigger, availability rule, and named test. Register §36I1-§36I7.
- **`DIRECTIONAL_CONTINUATION` researched and resolved as Outcome B — not deterministic, deferred:** neither `BTMM_STATE_MACHINE.md` nor `BTMM_MASTER_SUMMARY.md` nor any other knowledge-base file defines a rule, formula, or trigger for this BTMM-level cancellation reason beyond its bare name. The only defined "directional continuation" concept with an exact formula, `REJECTED_DIRECTIONAL_CONTINUATION` (Ambiguity 13), operates entirely within POI-candidate formation, before any `PoiObservation` or `BTMM_CANDIDATE` exists, and structurally cannot be the source of a BTMM-level cancellation. It is not added to the public enum; a static test enforces its absence. `MANUAL_REVIEW_REJECTED` receives the same Outcome-B treatment, since no field in the minimal reviewed-evidence contract carries a standalone reviewer-rejection signal. Register §36H4.
- **Same-group processing order, exact and closed:** availability-time-driven correctness by construction is now given one exact 10-step procedural model per global availability group, with an explicit primary-state-transition priority (`CANCELLED > CONFIRMED > BLOCKED > RESUMED_FORMING > ENTERED_FORMING`), a structural (not merely policy) guarantee that a newly created candidate cannot advance beyond `BTMM_CANDIDATE`/`BTMM_FORMING` in its own creation group, and explicit source-POI-invalidation priority. Register §36AA2.
- **Fingerprint strategy: Option B, final** — a fourth disclosed, local, tested-equivalent duplicate of the canonical serializer, matching `1B-J-POI`'s own precedent exactly; no completed analyzer (`domain/analyzer.py`, `structure/analyzer.py`, `poi/analyzer.py`) is modified; a required **four-way** cross-package equivalence test extends the three-way test `1B-J-POI` already established; the maintenance risk of four independent serializer copies is explicitly disclosed and accepted in exchange for zero blast radius on three closed milestones. This removes the original draft's Option A default and every remaining fingerprint-strategy uncertainty. Register §36AC.
- **Corrected public API:** `analyze_btmm(timeframe_inputs, poi_analysis, reviewed_evidence, configuration, identity_provider) -> BtmmAnalysis` — `reviewed_evidence` is now a required parameter (an empty tuple is always valid; no hidden manual-evidence repository, no global review registry, no wall clock, no I/O, no future-evidence use). Register §36AE.
- **Corrected contracts:** `BtmmObservation` unchanged at 15 fields; `CurrentBtmmState` corrected to 33 fields (retypes `volume_pillar_status`, adds `reviewed_evidence_availability_time_utc`); `BtmmLifecycleTransition` corrected to 16 fields (adds `blocked_reason` and `triggering_reviewed_evidence_availability_time_utc`, makes `triggering_candle_record_id` nullable); one new contract, `BtmmReviewedEvidence` (15 fields, a caller-supplied input, not an engine-generated `DerivedOutput`). Register §36G2/§36V-§36X.
- **Implementability matrix, corrected: 10 of 12 pattern components now IMPLEMENTABLE** (POI, Accuracy, Reaction, Reaction-Speed, Formation-Timeframe Gates, Liquidity-after-POI evidence, POI-invalidation inheritance, plus the now-reachable Context Gate, Volume Pillar Gate, and full Liquidity Gate via reviewed evidence); 2 DEFERRED (Accumulation/Distribution, Approach Speed, unaffected by this correction); 0 BLOCKED. Register §36AJ/§36AK.
- **Exact corrected scope:** 22 changed paths — 11 new source files in a new top-level package `btmm/` (adds `reviewed_evidence.py` to the original 10), 10 new test files, 1 modified existing file (`domain/enums.py`, +3 enum members). New-path split 11 source/10 test; affected-path split 12 source/10 test. 82 new top-level test functions, unchanged total, redistributed across the same 10 files (582 combined with the existing 500). **Corrected creation order 147-167.** **29 new public exports** (corrected from 28 in a subsequent narrow consistency pass, adding `BtmmLiquidityEvidenceStatus`; recounted directly). No dependency change; no `market_data` Protocol modification; `btmm/` remains the first package to depend on a previously-implemented milestone's own package (`poi`). Register §36AL-§36AN.
- **Narrow consistency correction (register §36I2/§36AA2/§36H1):** a subsequent focused review found and resolved two further issues in the consolidated correction above — (1) `ENTERED_FORMING` incorrectly fired in the same processing step as `BTMM_CANDIDATE` creation, contradicting the same-availability-group policy that a new candidate cannot advance beyond `BTMM_CANDIDATE` in its own creation group; corrected so `ENTERED_FORMING` is structurally deferred to the first later group in which a new confirmed candle becomes available, with a candidate genuinely invalidated beforehand now transitioning directly `BTMM_CANDIDATE → BTMM_CANCELLED`. (2) `liquidity_evidence_status` incorrectly reused the generic `BtmmGateStatus` (pass/fail) vocabulary for a field the approved standard never describes in pass/fail terms; corrected to a new, minimally-scoped `BtmmLiquidityEvidenceStatus` enum (`PENDING`/`PRESENT` only), which is the genuine, semantics-driven reason the export total becomes 29.
- **Author-approved, exactly as corrected — no modification was made to any approved element (register §36AT).** Every blocking and non-blocking finding from both the original audit (B1-B5, N1-N4) and the subsequent narrow consistency review was resolved before approval. This approval authorized implementation of rows 1-10 of the 12-row implementability matrix only. Full detail: register §36AT.
- **Implementation closure (register §36AU):** all 22 approved paths (11 new source files, 10 new test files, 1 modified existing file) were implemented in one complete cycle, passing every quality gate (`uv lock --check`, `ruff format --check .`, `ruff check .`, `mypy src tests`, full suite 660 passed, original baseline subset 34 passed, 82 new top-level test functions, 582 combined) with no dependency or `market_data`/`structure`/`poi` Protocol change.
- **Final architectural audit:** verdict `A — PASS, NO BLOCKING FINDINGS`. Genuine defects were found and corrected before commit — a drafting-error enum-member fix (`BtmmGateStatus.PASS_` → `PASS`), unused-import removal, an `EvidenceClassification` enum-value fix in place of a raw string, and a `CurrentBtmmState` sort-key correction to `btmm_setup_record_id` — none altering any architecture decision, contract, test count, export count, dependency, or Protocol. Test-code-only corrections (relaxed cancellation-reason assertion, a rewritten forward-only-evidence assertion, a deterministic content-addressed identity-provider test double, two missing return-type annotations) affected no production behavior. No other defect was found. Implemented and committed as `76d90402660b9accd9e0d4b462516e9250193e77`, pushed to `origin/main`. Full detail: register §36AU.
- **No network adapter, real file parser, production persistence backend, entry-signal, indicator, alert, backtester, or robot was implemented.** The scaffold has advanced from author approval (§36AT) to a completed, closed BTMM Manipulation Lifecycle Foundation, and no further.
- **Phase `1B-K-BTMM` is closed but not production-approved.** No production, live-trading, indicator, robot, provider-networking, or persistence-backend approval exists or is implied by this closure. **Next controlled action:** define the **Entry Validity and Trade Plan Foundation** (proposed identifier `1B-L-ENTRY`), using the completed market-data, measurement, structure-state, POI, and BTMM manipulation-lifecycle foundations — exact eligible BTMM states for entry evaluation, entry-trigger vocabulary, confirmation candle, reclaim/retest and displacement entry, lower-timeframe confirmation, structure prerequisites, entry availability, no-look-ahead, entry invalidation, cancellation before execution, entry price representation, stop-loss/take-profit/risk-reward logic, position sizing boundaries, multiple-entry handling, expiry, conflicting POIs/BTMM setups, replay equivalence, the paper-trading boundary, and the explicit separation of POI validity, BTMM validity, entry validity, trade-plan validity, execution status, and trade outcome — without yet including live broker execution, real-money order placement, MT4/MT5 connectivity, autonomous risk allocation, AI inference, or production approval. Not started by this closure. **Redirected — see §28 below.** A prerequisite entry-standards knowledge round was drafted and audited (verdict `C — CONSOLIDATED CORRECTION REQUIRED`); it was parked externally, unapproved, and the project instead proceeds next with the Scanner Integration and Historical Backtesting Foundation, which requires no entry/risk decision. This historical text is preserved unchanged and not rewritten.

## 28. Scanner Integration and Historical Backtesting Foundation — Architecture (Author-Approved)

**Status: `AUTHOR-APPROVED`, `APPROVED FOR CONTROLLED IMPLEMENTATION`, `NOT YET IMPLEMENTED`, `NOT PRODUCTION-APPROVED`.** Full detail: `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §38 (corrected)/§38Z (approval); `PHASE_1B_EXACT_SCAFFOLD_FILE_SCOPE.md` Section 43. Identifier `1B-L-SCANNER`. **Author-approved 2026-07-30:** *"I approve 1B-L-SCANNER — Scanner Integration and Historical Backtesting Foundation for controlled implementation. The milestone remains NOT PRODUCTION-APPROVED."*

This milestone completes the deterministic scanner — a pure orchestrator, not a fifth detector — over the four closed analytical foundations, before any trading-bot (entry/risk/execution) work begins. It requires no resolution of the still-unapproved `1B-L0-ENTRY-STANDARDS` draft.

- **Purpose and boundary:** `scan_market` calls `analyze_market_measurements`, `analyze_structure_state`, `analyze_pois`, and `analyze_btmm` in their already-approved dependency order and aggregates their already-identified outputs into one deterministic per-symbol `ScannerAnalysis`. No candle, measurement, structure, POI, or BTMM fact is redetected or duplicated. Verified against the actual implemented code: `analyze_market_measurements`/`analyze_structure_state` are single-timeframe; `analyze_pois`/`analyze_btmm` are multi-timeframe via `PoiTimeframeInput`/`BtmmTimeframeInput` (each carrying `measurement_analysis`, never `StructureAnalysis` — structure remains independent, not a POI/BTMM input, unchanged); `PoiAnalysis`/`BtmmAnalysis` carry no `record_id`/`content_fingerprint` — `ScannerAnalysis` follows the identical unidentified-aggregate pattern.
- **Orchestration order (13 exact steps, re-corrected — register §38B):** validate scanner inputs → normalize deterministic timeframe order → measurements for every supplied timeframe → structure for every supplied timeframe (exposed, not fed downstream) → build `PoiTimeframeInput` for **every** supplied timeframe, required and optional alike → one multi-timeframe `analyze_pois` call using all supplied timeframes → select only the BTMM-configured subset from the available analyses → build `BtmmTimeframeInput` for that subset only → availability-gate reviewed BTMM evidence → run `analyze_btmm` over the configured subset → build `ScannerSetupSummary` views → construct `ScannerAnalysis` → apply deterministic ordering.
- **Historical replay:** `run_scanner_replay` generalizes the already-implemented `InMemoryHistoricalReplaySource` single-timeframe atomic-availability-group grouping to a deterministic multi-timeframe k-way merge — a merge of already-sorted sequences, not new detection logic. Direct-batch-versus-replay equivalence requires exact equality at an identical visible prefix; **every** difference is unconditionally a hard mismatch, recorded as data (`DetectionMismatch`), never raised as an exception — no severity vocabulary exists, since no legitimate non-blocking difference is defined anywhere in this architecture (narrow correction, register §38G). **Historical-input capability, corrected wording (register §38E):** the `SourceCandleInput → NormalizedCandle` normalization pipeline is fully implemented and reusable; **no real FXCM/TradingView disk-file parser exists** (`OfflineFileSource` is fixture-mapping-based only, verified directly). The scanner is implementable and testable today using normalized historical candle tuples; actual exported-file parsing remains a separate, out-of-scope boundary task.
- **Timeframe policy (re-corrected, register §38B/§38D — resolves the focused-audit's BLOCKING finding B1):** `required_timeframes = {M1, M5, M15}`; `optional_timeframes = {H1, H3, H4, D1, W1}` are genuinely accepted and processed by measurements, structure, **and POI analysis** — verified directly against `poi/analyzer.py` that `analyze_pois` imposes no timeframe restriction of its own and legitimately requires higher timeframes for book-sourced period-level POIs and higher-timeframe candlestick patterns (the book's own "Daily > 4H > 1H > 15-minute" ranking). **Only BTMM analysis remains independently restricted** to its own configured subset (`BtmmConfiguration.formation_timeframes | supporting_only_timeframes`, default `{M1, M5, M15}`, verified in `btmm/configuration.py`). **Scanner timeframe acceptance does not equal BTMM timeframe eligibility.** No input is silently accepted with zero observable effect.
- **Identity-provider requirement, made explicit (register §38N):** direct-batch-versus-replay identity/fingerprint equivalence requires a content-addressed `DerivedOutputIdentityProvider` implementation — the public Protocol itself guarantees no such stability; a sequential/call-order-dependent provider is not valid for this comparison (inherited project-wide requirement, not new to the scanner).
- **Reviewed benchmark labels:** `ReviewedScannerCase`/`ExpectedPoiLabel`/`ExpectedBtmmLabel` are caller-supplied evaluation ground truth (no `record_id`/`content_fingerprint`/`evidence_classification`, matching `BtmmReviewedEvidence`'s own precedent), never entering scanner computation — only a separate `evaluate_scanner` function taking `(replay_result, reviewed_cases)`. No acceptance tolerance is invented; raw boundary error/timing delay is reported. Label-to-detection matching uses a deterministic five-key greedy zone-overlap-ratio ordering (not full bipartite matching) in V1, disclosed `ENGINEERING-PROVISIONAL`, and is proven structurally incapable of producing an ambiguous result (the two final tie-break keys are each a globally unique identifier) — no `AmbiguousLabelMatchError` is defined. Precision/recall is computed only when a case's own completeness flag is set for that concept; otherwise raw counts only.
- **Identity strategy: Option B** — no scanner-derived `DerivedOutputType`/identity; `ScannerAnalysis`/`ScannerSetupSummary` and every replay/evaluation-harness contract are unidentified aggregates, matching `PoiAnalysis`/`BtmmAnalysis`'s own precedent. No new `DerivedOutputType` member; no completed analyzer reopened.
- **Exact proposed scope (narrow correction — exact counts, not approximations):** 28 changed paths, all new (14 source, 14 test), 0 modified. New-path split 14/14; affected-path split identical. **Exactly 128 new top-level tests** (literal names locked, register §38X), combined with the existing 582: **exactly 710**. **Proposed creation order 168–195.** **Exactly 26 new public exports** (2 enums, 17 contracts, 4 errors, 3 APIs — corrected from an internally-inconsistent original claim of 28: `EvaluationConfiguration`, `DetectionMismatchSeverity`, and `AmbiguousLabelMatchError` were each found to lack distinct/reachable semantics and removed). No dependency change; no `market_data`/`domain`/`structure`/`poi`/`btmm` Protocol modification; every existing package is consumed only via its own already-public API.
- **Implementability matrix: 18 rows — 13 `IMPLEMENTABLE`/`IMPLEMENTABLE_WITH_AUTHOR_GAP_FILL`** (scanner orchestration, aggregate, setup summaries, historical replay, direct-batch equivalence, reviewed POI/BTMM labels, POI/BTMM matching, lifecycle matching, detection metrics, health metrics, JSON reporting via existing serialization), **3 `DEFERRED`** (CSV reporting, chart rendering, Telegram delivery), **2 `BLOCKED`** (profit backtesting — `P0G-B016`; entry/trade backtesting — `1B-L0-ENTRY-STANDARDS` unapproved).
- **Explicit exclusions (unchanged from the task's own list):** entry confirmation, planned entry, stop loss, take profit, position sizing, risk/reward, paper orders, broker execution, MT4/MT5, trade outcome, profit backtesting, AI inference, production approval.
- **Author-approved 2026-07-30, exactly as corrected — no modification was made to any corrected element.** No `scanner/` path exists. No dependency change. Inventory locked: 168 → 196 (28 new rows, `PHASE_1B_EXACT_SCAFFOLD_FILE_SCOPE.md` Section 9). This approval-and-inventory-lock record is documentation-only — no source code or test file is included.

**Next controlled action:** one complete implementation cycle covering all 28 approved paths in a single controlled batch (no per-file approval cycles), followed by one final architectural audit and, only if a genuine defect is found, at most one correction cycle, one implementation commit, and one compact closure commit. Explicitly keep deferred/blocked: CSV reporting, chart rendering, Telegram delivery, profit backtesting, entry/trade backtesting, live broker execution, AI inference, production approval. Not started by this record. **Superseded — see Section 29 below.** `1B-L-SCANNER` has since been implemented, committed, and pushed (`c94cd7b`); a post-implementation correction cycle then raised amendment `1B-L-SCANNER-A1`. This historical text is preserved unchanged.

## 29. `1B-L-SCANNER-A1` — Tick-Size Provenance Amendment (Author-Approved)

**Status: `AUTHOR-APPROVED`, `APPROVED FOR CONTROLLED IMPLEMENTATION`, `NOT YET IMPLEMENTED`, `NOT PRODUCTION-APPROVED`.** This supersedes the prior `ARCHITECT-RECOMMENDED`/`AUTHOR-DECISION REQUIRED` status. **Author-approved 2026-07-30:** *"I approve 1B-L-SCANNER-A1 — Tick-Size Provenance Amendment for controlled implementation. The milestone remains NOT PRODUCTION-APPROVED."* Full detail: `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §39 (recommendation §39A–§39G, approval record §39H). `1B-L-SCANNER` (§28 above) was implemented and pushed exactly as approved (commit `c94cd7b0e14c8a171c8918ccfac3f826df1d0b1e`). A post-implementation correction cycle then confirmed that `PoiValidationReport.mean_boundary_error_ticks` cannot be truthfully tick-normalized: `evaluate_scanner(replay_result, reviewed_cases)` and every contract reachable from those two arguments carry no `minimum_price_tick` field — that field exists only on `MarketMeasurementConfiguration`/`PoiConfiguration`/`BtmmConfiguration`, none of which is retained in scanner output.
...
## 31. Historical Dataset Ingestion and First Scanner Backtest (`1C-A-REAL-BACKTEST`) — Architecture (Architect-Recommended, Corrected)

**Status (historical — superseded by author approval): `ARCHITECT-RECOMMENDED`, `AUTHOR-DECISION REQUIRED`, `NOT YET IMPLEMENTED`, `NOT PRODUCTION-APPROVED`.** **Current status: `AUTHOR-APPROVED`, `APPROVED FOR CONTROLLED IMPLEMENTATION`, `NOT YET IMPLEMENTED`, `NOT PRODUCTION-APPROVED`.** **Author-approved 2026-07-31:** *"I approve 1C-A-REAL-BACKTEST — Historical Dataset Ingestion and First Scanner Backtest for controlled implementation. The milestone remains NOT PRODUCTION-APPROVED."* Full detail: `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §41 (consolidated correction pass, then the final narrow completion-marker correction). Baseline: `HEAD == origin/main == 76aa2e56e4b2a259edf5d4d46039985bba7a7963`, both `1B-L-SCANNER` and `1B-L-SCANNER-A1` formally closed.

**Purpose:** a disk-file → `SourceCandleInput` → `NormalizedCandle` historical loader (reusing the existing normalization pipeline unmodified), a dataset manifest/provenance layer (`DatasetManifest`, `HistoricalFileEntry`, `HeaderMappingEntry`), a multi-symbol backtest orchestrator over the already-implemented `run_scanner_replay`/`evaluate_scanner`, and a deterministic outer JSON report writer — the first real POI/BTMM backtest, not a new detector. Excludes entry confirmation, entry price, stop loss, take profit, risk/reward, position sizing, profit/loss, drawdown, expectancy, broker execution, MT4/MT5, live provider connection, Telegram, chart rendering, AI training, and production approval.

**Corrections applied in the prior pass (unchanged, carried forward):** (1) the header-mapping policy no longer relies on any built-in `TRADINGVIEW`/`FXCM` alias table — each `HistoricalFileEntry` instead carries an explicit, manifest-authored `header_mapping: tuple[HeaderMappingEntry, ...]`; (2) timestamp/availability derivation is DST-safe and timeframe-class-specific; (3) the "exact first-dataset calendar range" decision is withdrawn and replaced by a fully objective, configuration-derived history-sufficiency formula (register §41R); (4) CLI exit codes are locked to an exact 7-tier policy with earliest-failing-stage precedence; (5) the full 28-item public export list is locked; (6) the implementability matrix arithmetic was corrected and re-verified.

**Corrections applied in this pass, responding to the focused read-only audit's verdict `C — CONSOLIDATED CORRECTION REQUIRED`:** (1) candle-level `provenance_id`/`record_id`/`content_fingerprint`/`source_record_id` derivation is now fully specified as a private mechanism (`historical_backtest/identity.py`, register §41O2), separate from `ContentAddressedIdentityProvider` (which cannot own candle identity, since `DerivedOutputType` has no candle-level member) — no new public surface; (2) the DST ambiguous/nonexistent-time detection is now an exact, implementable fold=0/fold=1 UTC round-trip algorithm (register §41G2); (3) path lexical validation and filesystem symlink-escape/resolved-root-containment security are now fully specified (register §41C2); (4) `D1`/`W1` session-close derivation now uses two new explicit, mandatory `HistoricalFileEntry` fields (`calendar_close_day_offset`/`calendar_close_time_local`, register §41G3/§41E) instead of assuming the open timestamp already encodes the broker's session convention; (5) checksum byte-exactness is locked to raw on-disk, pre-parse bytes (register §41K2); (6) the JSON writer's atomicity claim is corrected to per-file replacement only, never whole-directory publication, with an explicit completion-marker rule (register §41T). **Contract field-count corrections:** `HistoricalFileEntry` 13 → **16** fields; `DatasetManifest` 16 → **17** fields (`reviewed_case_sha256` added, register §41D). **Test-plan correction:** 9 new tests added within the existing 13 test files, 59 → **68** named tests (register §41AA).

**Proposed scope (corrected, file/path counts unchanged by this pass):** new package `src/btmm_ai_scanner/historical_backtest/`, 12 source + **13** test files, **25** total paths, 0 modified — nothing created by this section. Inventory unchanged at 196 now; **+25** projected upon future approved implementation (196 → **221**).

**Implementability matrix (unchanged by this pass, 20 rows):** `IMPLEMENTABLE` = 16 (rows 1–16); `IMPLEMENTABLE_WITH_AUTHOR_GAP_FILL` = 1 (row 17, first real backtest — needs a real exported dataset file in hand, not an architecture decision); `DEFERRED` = 1 (row 18, `H3` resampling); `BLOCKED` = 2 (rows 19–20, live ingestion, profit backtesting). Total = 16+1+1+2 = 20.

**Exact test/export totals (unchanged by the final narrow pass below):** 68 named top-level tests (up from 59, adding candle-identity/timestamp-format/symlink-security/session-close test ownership); AST future total 710+68=**778**; pytest-collected future total 788+68=**856**; 28-item exact public export list (7 enums, 12 contracts, 4 errors, 4 APIs, 1 identity implementation, unchanged).

**Final narrow correction (register §41T2): the JSON writer's completion marker is corrected from `execution_summary.json` to a dedicated `checksums.json`.** `execution_summary.json` was mistakenly named the completion marker by the immediately prior correction pass — it is in fact an ordinary report file, indistinguishable in kind from `manifest_echo.json`/`data_quality_report.json`/etc. `checksums.json` (5 keys, no new public contract) is written and finalized strictly last; its presence and validity is the sole signal a reader may use to treat an execution directory as complete. This adds no new path, contract, export, or test file — verification is assigned to the two existing reporting tests already covering atomic writing and failure cleanup; all totals above (68 tests, 25 paths, 28 exports) are unchanged.

**Final architecture verdict:** `B — CORRECTED WITH NON-BLOCKING FINDINGS — READY FOR AUTHOR APPROVAL` (upgraded from the focused audit's verdict `C`). Every one of the four blocking findings (candle identity, DST algorithm, path/symlink security, D1/W1 session-close) and both non-blocking precision findings (checksum bytes, writer atomicity) is resolved by an explicit rule. Exactly one disclosed non-blocking gap remains, unchanged (row 17: a real dataset file must exist and be reviewer-confirmed against the objective history-sufficiency rule — an external artifact, not a decision). No mandatory stop condition is triggered (full check: register §41AC). **Not yet implemented.** This record is documentation-and-architecture-only; no scanner/market-data package, test, dependency, lockfile, Protocol, or inventory entry is affected. Remains `NOT PRODUCTION-APPROVED`.

## 32. `1C-A-REAL-BACKTEST-A1` — Deterministic IANA Time-Zone Data Dependency Amendment (Author-Approved)

**Status: `AUTHOR-APPROVED`, `APPROVED FOR CONTROLLED IMPLEMENTATION`, `NOT YET IMPLEMENTED`, `NOT PRODUCTION-APPROVED`.** This supersedes the prior `ARCHITECT-RECOMMENDED`/`AUTHOR-DECISION REQUIRED` status. **Author-approved 2026-08-01:** *"I approve 1C-A-REAL-BACKTEST-A1 — Deterministic IANA Time-Zone Data Dependency Amendment for controlled implementation, including tzdata==2026.3 as a direct runtime dependency and authorized changes to pyproject.toml and uv.lock. The milestone remains NOT PRODUCTION-APPROVED."* Full detail: `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §42. Implementation of §31 began (12 of 25 approved source paths written) and was interrupted upon discovering that `zoneinfo` cannot resolve any IANA zone, including `"UTC"`, on this Windows host without either a system tz database (absent) or the `tzdata` PyPI package (not installed) — a genuine contradiction between the approved DST/timezone requirements (§41G–§41G3) and the approved "standard library only" dependency policy (§41Z).

**Approved-for-recommendation resolution:** add `tzdata==2026.3` as a direct runtime dependency (first-party, required on every platform), consumed exclusively through a private, bundled-only loader (`ZoneInfo.from_file` against the installed `tzdata` package's own resources via `importlib.resources` — never the host's own `TZPATH`/system database). The already-approved fold-based DST resolver, and real `America/New_York`-based DST testing, are entirely preserved unchanged. No new public export; no change to any locked contract, enum, or test name.

**Scope:** authorizes future modification of exactly `pyproject.toml` and `uv.lock` (2 existing files) in addition to the already-approved 25 new paths — projected future total **27 affected paths**. Inventory (196 → 221), test count (68), and export count (28) are all unchanged.
