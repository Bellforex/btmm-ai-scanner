...
| 1B-C-MD | `src/btmm_ai_scanner/market_data/__init__.py` | New | Python | Marks `market_data` as a package; re-exports the exact 20-name approved public API | Phase 1B-C Decision Group 2 §29J | Exact 20-name `__all__` list per §29J, exact order | Any private helper export; any export beyond the 20 named symbols | All 8 sibling source files | Import smoke test (`test_ingestion_ports.py`/`test_historical_live_separation_and_no_synthetic_fabrication.py` exercise imports indirectly) | None — resolved and author-approved by Decision Group 2 (§29O) | 66 | **AUTHOR-APPROVED SCOPE** (§28); **IMPLEMENTED, VERIFIED, COMMITTED, PUSHED, CLOSED** (§30 — commit `d328776abb5a2c1f42e185b9bc80f0e5a371897e`), **NOT PRODUCTION-APPROVED** |
...
**Total proposed changed paths: 69** (68 new files + 1 modified existing file, `.gitignore`, added by the Phase 1B-A Decision Group 3 — Metadata, Configuration Contract, Tests and Git Hygiene correction). `.python-version`'s *inclusion and content* are both fully resolved (`3.12.13`). **`.gitignore` had no prior row in this inventory — this is a newly added row, not a reclassification of an existing one.** **None is created or modified by this document.** Every row above is either "Required in initial scaffold" (Batch 1B-A, 10 changed paths: 9 new files + 1 modified `.gitignore`), "Required in later foundation batch" (Batches 1B-B through 1B-F, 42 files — Batch 1B-B 15 files, Batch 1B-C (validation/eligibility) 12 files, Batch 1B-D 7 files, Batch 1B-E 7 files, Batch 1B-F 1 file), "**`AUTHOR-APPROVED SCOPE` pending implementation-control approval**" (Batch `1B-C-MD`, the market-data pipeline, 17 changed paths — new files only, no modified file), or explicitly excluded as "Deferred beyond Phase 1B" (manifests, raw-storage I/O, provider adapters — see Section 6) or "Prohibited" (any entry/risk/execution file — none appears above). **No file listed above is marked as already created or modified except the 17 Batch 1B-B paths and the 10 Batch 1B-A paths already implemented and closed — every Batch `1B-C-MD` row remains a proposal only.**
