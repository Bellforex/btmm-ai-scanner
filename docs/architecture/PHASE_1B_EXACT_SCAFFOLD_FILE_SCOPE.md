...
| 1B-C-MD | Market-Data Pipeline (First Batch) — Phase 1B-C Decision Group 2 (`PHASE_1B_AUTHOR_DECISION_REGISTER.md` §29) | **17 changed paths** (Section 9): 17 new files, no modified file — `AUTHOR-APPROVED implementation controls (§29O), NOT YET IMPLEMENTED` | 1B-B (`contracts/`, `config/enums.py`) only — does **not** depend on 1B-C (validation/eligibility), 1B-D, 1B-E, or 1B-F | None — Phase 1B-C Decision Group 2 itself (§29) is now `AUTHOR-APPROVED` in full (§29O), following a final read-only architectural audit verdict of `A. PASS — READY FOR AUTHOR APPROVAL`; AT-1/RM-1/GAP-1 already `AUTHOR-APPROVED` (§28M) | Availability-decision-matrix layering error, idempotency edge-case gap, or gap-interval miscalculation would require a pre-implementation correction, mirroring Batch 1B-B's decision-group correction rounds | 57 unit tests across 8 files (§29K) | Full quality-gate pass (§29M); exactly 57 new + 132 existing = 189 combined top-level test functions; exact 20-name `__init__.py` export order (§29J); no 18th path | No commit exists yet — rollback boundary is documentation-only (§29M): any future rollback affects only these 17 paths, never `pyproject.toml`/`uv.lock`, never a Batch 1B-A/1B-B path | NOT YET AUTHORIZED FOR EXECUTION — requires a separate, explicit implementation authorization naming Decision Group 2, distinct from the `AUTHORIZE PHASE 1B-B IMPLEMENTATION` phrase used for Batch 1B-B; architecture and implementation controls are now fully author-approved, but implementation itself has not been separately authorized |

**Total across all batches: 69 changed paths** (10 + 15 + 12 + 7 + 7 + 1 + 17 = 69), matching Section 9's corrected inventory total. **Batch `1B-C-MD` (17 paths) is `AUTHOR-APPROVED SCOPE` at the architecture level (§28N) and its implementation-control detail (§29) is now `AUTHOR-APPROVED` (§29O) — it is not yet authorized for execution (a separate, explicit implementation authorization remains required) and is not counted among the 52 previously-inventoried-and-partially-implemented paths.**
