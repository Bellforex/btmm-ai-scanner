...
| 1B-B | `src/btmm_ai_scanner/contracts/types.py` | New | Python (Pydantic v2) | UUIDv7 identity type (validation-only), SHA-256 fingerprint type (validation-only), project-owned SemVer version type | Group 4 (identity, fingerprint, versioning); Phase 1B-B Decision Group 1 (`PHASE_1B_AUTHOR_DECISION_REGISTER.md` §19) | Value-type wrappers only — **Pydantic v2 required, dataclass placeholder rejected**; UUIDv7 validation-only (no generation); `SHA256Fingerprint` validation-only (64-char lowercase hex, no generation); project-owned `SemVer` type (exact grammar/parsing/comparison unresolved, §19E) | Any entry/risk field; any UUID/fingerprint *generation* logic; any external UUIDv7 or SemVer package | `pydantic>=2.13.4,<2.14` (approved, not yet added) | `test_identity_and_fingerprint.py`, `test_semver.py` | #20, #21 (canonical serialization / field sets — RESOLVED FOR BATCH 1B-B SCOPE as "not this batch's concern," per §19C/§19D); exact SemVer grammar/parsing/comparison (§19E, still open) | 11 | Required in later foundation batch |
