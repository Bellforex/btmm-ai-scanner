...
**Total across all batches: 52 changed paths** (10 + 15 + 12 + 7 + 7 + 1 = 52), matching Section 9's corrected inventory total.
...
**Open author decisions, each now an architect-recommended resolution awaiting author approval (not an open question):** AT-1 (availability-time-quality representation — `SourceCandleInput` requires explicit availability fields; missing evidence yields `INDETERMINATE` before any `RawCandle` is built); RM-1 (conflicting-revision resolution policy — quarantined, no automatic winner, no silent overwrite); GAP-1 (trading-session/calendar boundary — `POTENTIAL_GAP` only in the first batch) — full detail in `PHASE_1B_AUTHOR_DECISION_REGISTER.md` §28M. The Decision Group itself remains `ARCHITECT-RECOMMENDED`, `AUTHOR-DECISION REQUIRED`, `NOT YET IMPLEMENTED` — not approved.
