...
The author's standing complaint is that dead zones keep looking tradeable. So
this is ONE predicate for every lifecycle-eligible family -- not a rule per POI
type -- and it is meant to govern the zone box, the label and the active table
row together. Hiding a label while leaving the rectangle on the chart is
exactly the failure being fixed.

Nothing is deleted. History keeps every record; this is a view.
"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _State(terminal_reason=PoiTerminalReason.MITIGATED),
...
def test_the_contract_holds_for_every_family_not_just_one(analysis) -> None:
    ...
    for observation in result.poi_observations:
        ...
        assert not is_rc5_active_for_display(observation, mitigated, ledger)
