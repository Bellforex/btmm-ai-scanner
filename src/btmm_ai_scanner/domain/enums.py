from enum import StrEnum


class SwingType(StrEnum):
    SWING_HIGH = "SWING_HIGH"
    SWING_LOW = "SWING_LOW"


class DisplacementDirection(StrEnum):
    BULLISH = "BULLISH"
    BEARISH = "BEARISH"


class DisplacementClassification(StrEnum):
    NORMAL = "NORMAL"
    FAST = "FAST"
    VERY_FAST = "VERY_FAST"


class EqualLevelType(StrEnum):
    EQUAL_HIGH = "EQUAL_HIGH"
    EQUAL_LOW = "EQUAL_LOW"


class SupportResistanceType(StrEnum):
    SUPPORT = "SUPPORT"
    RESISTANCE = "RESISTANCE"


class TrendlineOrientation(StrEnum):
    BULLISH_TRENDLINE = "BULLISH_TRENDLINE"
    BEARISH_TRENDLINE = "BEARISH_TRENDLINE"


class DerivedOutputType(StrEnum):
    CONFIRMED_SWING = "CONFIRMED_SWING"
    DISPLACEMENT_OBSERVATION = "DISPLACEMENT_OBSERVATION"
    EQUAL_LEVEL_CLUSTER = "EQUAL_LEVEL_CLUSTER"
    SUPPORT_RESISTANCE_ZONE = "SUPPORT_RESISTANCE_ZONE"
    TRENDLINE = "TRENDLINE"
    SWING_RELATIONSHIP = "SWING_RELATIONSHIP"
    STRUCTURE_TRANSITION = "STRUCTURE_TRANSITION"
    CURRENT_STRUCTURE_STATE = "CURRENT_STRUCTURE_STATE"
    POI_OBSERVATION = "POI_OBSERVATION"
    POI_LIFECYCLE_TRANSITION = "POI_LIFECYCLE_TRANSITION"
    CURRENT_POI_STATE = "CURRENT_POI_STATE"
    BTMM_OBSERVATION = "BTMM_OBSERVATION"
    BTMM_LIFECYCLE_TRANSITION = "BTMM_LIFECYCLE_TRANSITION"
    CURRENT_BTMM_STATE = "CURRENT_BTMM_STATE"
