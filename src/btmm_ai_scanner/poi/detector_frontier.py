...
class _DetectorFrontierState:
    ...
    # RC3 OB leg origin: raw two-candle OB formations (append-only, NOT in the
    # universe), the leg-origin frontier and the ORDER BLOCKs it has locked
    # (append-only: a confirmed ORDER BLOCK never disappears).
    raw_order_blocks: tuple[Any, ...] = ()
    leg_origin: LegOriginFrontier = field(default_factory=LegOriginFrontier)
...
def _evaluate_new_bases(
    ...
    for length in range(
        ...
        if max_base_range == 0:
            continue
        # THE ONE Base-candle size rule, identical to detect_bases. The two must
        # never disagree, and test_base_family_ownership_incremental_equality
        # proves they do not at every prefix.
        if (
            max_base_range
            > configuration.base_candle_size_ratio_standard * departure_range
        ):
            continue
        ...
        strong = (
            max_base_range
            <= configuration.base_candle_size_ratio_strong * departure_range
        )
        ...
        results.append(
            BaseCandidate(
                ...
                # The arrival leg is NOT candle geometry, so neither path can
                # read it here -- it is assigned downstream from the causal
                # structure timeline. Emitting None on both paths is what keeps
                # them trivially equal on this field.
                base_family=None,
            )
        )
...
def _new_local_candidates(
    ...
    if len(ring) >= 2:
        # Raw OB formations are gated separately (leg origin); only the
...
def advance_detector_frontier(
    ...
    *,
    ...
    if new_origin_order_blocks is not state.origin_order_blocks:
        # Append-only by construction: only the newly locked tail is NEW.
        origin_new = list(new_origin_order_blocks[len(state.origin_order_blocks) :])
